<?php
// Attendance for MCA terms — take and review attendance per course
...
// This page mirrors attendance_bca but for MCA table names

// Create attendance table if not exists (shared with BCA)
...
// Handle AJAX load attendance (return existing records)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_attendance') {
    ...
    if ($s) {
        ...
        if (mysqli_stmt_fetch($s)) {
            if (isInstructorMatch($instructor, $currentUser, $currentUserId)) $canSubmit = true;
        }
        mysqli_stmt_close($s);
    }
...
<head>
    ...
    <title>Attendance — MCA</title>
    <link rel="stylesheet" href="styles/main.css">
    <link rel="stylesheet" href="styles/internal.css">
    <style>
    .att-modal { position:fixed; inset:0; display:none; align-items:center; justify-content:center; background:rgba(0,0,0,0.4); z-index:50 }
    .att-modal .panel { width:760px; max-width:96%; background:var(--surface); padding:16px; border-radius:10px; }
    .att-actions { display:flex; gap:8px; justify-content:flex-end; margin-top:12px }
    </style>
...
<body>
    <div class="container">
        <header class="page-header">
            <div>
                <h1 class="page-title">Attendance — MCA</h1>
                    ...
                    <?php
                        ...
                        if (!$courses || (is_object($courses) && mysqli_num_rows($courses) === 0)) {
                            ...
                            while ($course = mysqli_fetch_assoc($courses)) {
                                ...
                                $studentTable = 'mca_student_' . preg_replace('/^mca_/', '', $tableName);
                                ...
                                $canEdit = false; if (isInstructorMatch($instructor, $currentUser, $currentUserId)) $canEdit = true;
                                if ($canEdit) {
    ...
    <div class="att-modal" id="att-modal">
        <div class="panel">
            ...
            <div style="margin-top:8px;">
                <label>Date: <input type="date" id="att-date" value="<?php echo date('Y-m-d'); ?>" onchange="reloadAttendanceForDate()"></label>
            </div>
    ...
    (function(){
        ...
        document.querySelectorAll('.take-att').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                } catch (e) { /* ignore load errors, allow new entry */ }
