...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add' || $action === 'edit' || $action === 'delete') {
        ...
        if (!empty($errors)) {
            $message = "Missing/Invalid: " . implode(', ', $errors);
            $messageType = "error";
        } else {
            $tableName = "bca_student_" . preg_replace('/[^a-z]/', '', $term);
            $userId = $_SESSION['user_id'];
            
            // For add action, check if exam_roll_no already exists
            if ($action === 'add') {
                ...
                if ($duplicateInTable) {
                    ...
                    if ($duplicatePending) {
                        ...
                        // Insert a new add task (we allow multiple add requests by the same user)
...
<body>
    <div class="container">
        <header class="page-header">
            <div><h1>BCA Students</h1><p class="muted">Manage students</p></div>
        ...
        <main>
            ...
            ?>
                <section class="section-card card">
                    ...
                    <?php renderTable(getStudents($conn, "bca_student_$code"), "bca_student_$code"); ?>
                </section>
                <hr>
            <?php endforeach; ?>
        </main>
